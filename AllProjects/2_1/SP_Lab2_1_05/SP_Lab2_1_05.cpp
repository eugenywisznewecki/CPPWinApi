...
TEXT("C:\\Windows\\Notepad.exe"),
TEXT("TestProc.exe") };//для указателей строк, идентифицирущих файлы запускаемых программ;					:\\3trimestr\\sp\\Labi\\SP_Lab2_1\\SP_Lab2_1_05
LPTSTR CmdParam[5] = { 0, 0, TEXT("C:\\Windows\\Notepad.exe SP_Lab2_1_05.cpp"), 0, TEXT("") };; //для строк c параметрами запускаемых программ.
...
void WndProc_OnCommand(HWND hWnd, int id, HWND hwndCtrl, UINT codeNotify)
{
	...
	case CLOSE_NOTE: //закрытие бл
		//TerminateProcess(ProcHandle[1], exitCode);
		//TerminateThread(ThreadHandle[1], exitCode);
...
// Диалоговое окно "Сведения о процессе"
