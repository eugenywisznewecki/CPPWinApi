...
void WndProc_OnCommand(HWND hWnd, int id, HWND hwndCtrl, UINT codeNotify)
{
	...
	case CREATE_MUTEX:
		...
		IsMutex = true;
		EnableMenuItem(hMenu, CREATE_MUTEX, MFS_GRAYED);
		EnableMenuItem(hMenu, DESTROY_MUTEX, MFS_ENABLED);
		break;
	...
	case DESTROY_MUTEX:
		CloseHandle(hMutex);
		IsMutex = false;
		EnableMenuItem(hMenu, CREATE_MUTEX, MFS_ENABLED);
		EnableMenuItem(hMenu, DESTROY_MUTEX, MFS_GRAYED);
		//завершение всех потоков, работающих с мьютекстом
		SendMessage(hWnd, WM_COMMAND, (WPARAM)DESTROY_THREAD_1, 0);
		SendMessage(hWnd, WM_COMMAND, (WPARAM)DESTROY_THREAD_2, 0);
		break;
		//о приложении
	case ABOUT_APP:
...
DWORD WINAPI ThreadString(PVOID pvParam)
{
	...
	RECT  rc, rcInfo;
	HDC   hDC;
	int N = 2;							//общее количество пробегов строки
	int counter = 0;
	int quantity = 0; //общее количество пробегов строки

	// Формирование текста бегущей строки
	wsprintf(CreepingLine,
		TEXT("                                                                                                                                     Вторичный поток создал <Гайчук>"));
	StringLength = iBeginningIndex = lstrlen(CreepingLine);// Длина строки
	lstrcpy(buf, CreepingLine);//копирование строки вывода в рабочий строковый буфер
	...
	rcInfo.top = Param->yPos;
	rcInfo.left = Param->xPos - 120;
	rcInfo.right = rcInfo.left + 100;
	rcInfo.bottom = Param->yPos + 20;

	// Получение контекста для вывода строки
	hDC = GetDC(hWindow);

	// Бесконечный цикл вывода строк сериями по N строк
	while (true)// Цикл вывода серии из N строк
	{
		int cRun = 0; // обнуляем cчетчик “пробегов”
		//проверяем, включен ли мьютекс
		if(IsMutex)
		...
		while(cRun < N)
		{
			...
			for (int j = 0; j < StringLength; j++)
			{
				...
				for (int i = StringLength; i > 0; i--)//сдвигаем строку на один символ вперед
					buf[i] = buf[i - 1]; //переставляем предыдущий символ на место текущего

				buf[0] = c; //последний символ становится первым

				++counter; //увеличиваем количество выводимых символов (изначально 0)

				// Вывод строки
				DrawText(hDC, buf, lstrlen(buf) + 1, &rc, DT_LEFT | DT_SINGLELINE);
				
				//не сработало
				//int k = lstrlen(buf);
				//DrawText(hDC, buf, k, &rc, DT_LEFT | DT_SINGLELINE);

				iBeginningIndex--; //уменьшаем индекс начала строки

				Sleep(20); // приостановка потока  - замедление/задержка цикла
			} // Конец цикла полного однократного “пробега” строки

			cRun++; // Подсчет количества полных пробегов строки
			quantity++;

			//отображает количество проходов строки
			TCHAR infoBuf[100] = {0};
			wsprintf(infoBuf, TEXT("Проходов: %.2d"), quantity);
			DrawText(hDC, infoBuf, lstrlen(infoBuf), &rcInfo, DT_LEFT | DT_SINGLELINE);
		...
		{
			ReleaseMutex(hMutex);
			//DrawText(hDC, TEXT("                                                                                                                                                                                                                    "), 200, &rc, DT_LEFT | DT_SINGLELINE);
		}		 
	}
